...
class MaskGIT(nn.Module):
    ...
    def generate_random_mask(self, seq: torch.Tensor) -> torch.BoolTensor:
        ...
        num_masked_tokens = torch.randint(1, L + 1, (B, 1)).to(self.device) # random number of tokens we want to mask between 1 and L

        rand_scores = torch.rand(B, L).to(self.device) # each token has now a random score

        ranks = rand_scores.argsort(dim=1).argsort(dim=1) # we sort the tokens based on their random score

        random_mask = ranks < num_masked_tokens # if a token's rank is less than the number of tokens we want to mask, make it True
    ...
    def get_maskgit_schedule(self, mask: torch.BoolTensor, num_steps: int = 8) -> List[int]:
        ...
        constant = total_tokens // num_steps
        remainder = total_tokens - (constant * num_steps)
        schedule = [constant for i in range(num_steps)]
    ...
    def generate(
        ...
        for step, k in enumerate(schedule):
            ...
            _, top_masked = torch.topk(confidence, k)
            selected_positions = masked_indices[top_masked]
