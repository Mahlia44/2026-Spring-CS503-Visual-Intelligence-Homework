...
class GPT(nn.Module):
    ...
    def forward_model(self, x: torch.LongTensor) -> torch.Tensor:
        ...
        mask_tensor = torch.ones([L, L], dtype=torch.bool).to(self.device)
        causal_mask = torch.tril(mask_tensor).unsqueeze(0)
    ...
    def generate(
        ...
        for _ in range(self.max_seq_len - len(context)):
            ...
            last_token, _ = sample_tokens(logits[:, -1, :], temperature=temp, top_k=top_k, top_p=top_p)

            # Concatenate the new token to the current_tokens sequence
            current_tokens = torch.cat([current_tokens, last_token.unsqueeze(1)], dim=1)

            # Break if the end-of-sequence token is generated
            if last_token.item() == eos_idx: break
